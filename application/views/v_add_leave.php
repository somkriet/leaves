   ...
   //   })
	...
	$("#save_leave").click(function(){
		...
		$("#overlay").show();
		var ra = confirm("ยืนยันบันทึกการลาหรือไม่?");
		if (ra != true){
    		$("#overlay").hide();
  			return false;
  		}
  		var counttable = $("#dtable tr").length;
  		// alert(counttable);
  		if(counttable == 0){
  			alert("กรุณาเลือกวันที่ลาใหม่!!");
  			$("#overlay").hide();
  			return false;
  		}
	...
	function runleave_type_casual(value){
		...
		var currentYear = (new Date).getFullYear();
		if(type_id == '5'){
			$( "#txt_date_from" ).datepicker( "option", "maxDate", new Date());
        }else{
        	 var maxDate = new Date(currentYear+1, 3-1, 31);
        	$( "#txt_date_from" ).datepicker( "option", "maxDate", maxDate);
        }
...
function rundate(){
	...
	function cancelsat(id,thiss){
		...
		if(r==true){
			...
			var timeto = $("#timeto"+subid).val();
			var timefrom = $("#timefrom"+subid).val();
			var totalday = $("#totalday").val();
			// if(day == 9){
			if((timefrom == '08.30' || timefrom == '8.30') && timeto == '17.30'){
			...
			for(var i=0; i<count; i++){
				...
				$('#dtable tbody tr').eq(i).find('button').attr('id','cancel'+i);
				// $('#dtable tbody tr').eq(i).find('td').eq(0).text(id);
