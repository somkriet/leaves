			...
			<table id"stable" class="table table-condensed table-hover table-bordered">
				...
				<tbody>
					...
					<tr class="active">
					    ...
					    <?php if($row->leave_attached != "" AND $row->leave_attached != 0 AND $row->leave_type_ID != "5"){?> 
								<a href="<?php echo base_url('assets/upload/')."/".$row->leave_attached;?>" style="color:green" target="_blank">ไฟล์แนบ</a> 
						<?php }else if($row->leave_attached != "" AND $row->leave_attached != 0 AND $row->leave_type_ID == "5"){ ?>
								<a href="<?php echo base_url('assets/upload/')."/".$row->leave_attached;?>" style="color:green" target="_blank">มีใบรับรองแพทย์</a> 
						<?php }else if(($row->leave_attached == "" OR $row->leave_attached == 0) AND $row->leave_type_ID == "5"){ ?>
								<a style="color:red" id="<?php echo $row->leave_ID;?>" onclick="sendleaveid(this.id)">ไม่มีใบรับรองแพทย์</a> 
						<?php } ?>
					    </td>
						<td><a id="<?php echo $row->leave_ID;?>" onclick="send_leave_id(this.id,'<?php echo $row->leave_ID;?>','<?php echo $row->subject;?>','<?php echo $row->detail;?>')"><?php
        ...
        <div class="modal fade" id="leave_detail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
          <div class="modal-dialog" role="document" >
            <div class="modal-content" style="width:800px; height:700px;">
              ...
              <table id="detail_table" class="table-condensed table-hover ">
...
<form id="frmUpload" action="<?php echo base_url('index.php/leave_detail/');?>" method="post" enctype="multipart/form-data" >
<button type="button" class="addpic" data-toggle="modal" data-target="#addpic" style="display:none"></button>
        <div class="modal fade" id="addpic" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
          <div class="modal-dialog" role="document" >
            ...
            <div class="modal-content" style="width:500px; height:200px;">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">แนบไฟล์ใบรับรองแพทย์</h4>
              </div>
              <div class="modal-body" style="position: relative; padding: 15px;">
								<div class="col-md-12 col-xs-12 form-group">
									<label>ไฟล์แนบ / Attachment :</label>
									<input type="text" name="idleave" id="idleave" value="" style="display:none">
									<input type="file" class="form-control" id="file_upload" name="file_upload">
    								<!-- <input type="submit" name="btnSubmit" id="btnSubmit" value="อัพโหลด"  /> -->
								</div>
					
          	   </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="submit" name="btnSubmit" id="btnSubmit" value="อัพโหลด" class="btn btn-primary">Upload</button>
              </div>
            </div>
           <!-- </form> -->
          </div>
...
function send_leave_id(id,leave_id,subject,detail){
	$('#detail_table').empty();

	$('#detail_table').append('<tr><td width="30%"><b>หมายเลขการลา / Leave ID :</b></td><td width="70%">'+leave_id+'</td></tr>');
	$('#detail_table').append('<tr><td width="30%"><b>เรื่อง / Subject :</b></td><td width="70%">'+subject+'</td></tr>');
	$('#detail_table').append('<tr><td width="30%"><b>รายละเอียด / Detail :</b></td><td width="70%">'+detail+'</td></tr>');
  	$.ajax({
    type: "POST",
    dataType: "JSON",
